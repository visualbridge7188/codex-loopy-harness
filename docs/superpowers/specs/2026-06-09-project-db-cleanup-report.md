...
2026-06-09에 Project DB 2차 정리를 실행했다.

실행한 변경:

1. 다음 Project를 `진행 상황 = 완료`, `Hide = true`로 변경했다.
   - `051 : 올스테이트 추가페이지`
   - `26송정-학생선수관리`
   - `쓰레드 수익화 마스터 1기 (플랫폼트리)`
   - `CRM`
   - `N블로그`
2. 다음 Project의 `상위 프로젝트`를 `그로쓰/연구`로 설정했다.
   - `정부지원사업`
   - `헤르메스 Life OS`
   - `AI 에이전트`
3. `온디` 컨테이너성 Project를 새로 만들었다.
4. 다음 Project의 `상위 프로젝트`를 `온디`로 설정했다.
   - `온디 브랜딩`
   - `체험단&온디 글쓰기`
...
1. `그로쓰/연구`는 하위 Project 5개를 가진다.
2. `온디`는 하위 Project 2개를 가진다.
3. `온디 브랜딩`, `체험단&온디 글쓰기`는 `온디`를 상위 Project로 가진다.

운영 메모:

1. `WEB`은 작업을 받은 뒤 진행시키는 Project들을 묶는 상위 컨테이너로 본다.
2. 상위 컨테이너성 Project는 일반 목표형 Project처럼 완료 처리하지 않아도 된다.

---

## 11. 컨테이너 status 실행 기록

2026-06-09에 새 속성 추가 없이 기존 `진행 상황` status에 `컨테이너` 옵션을 추가했다.

실행한 변경:

1. `진행 상황` status option에 `컨테이너`를 추가했다.
2. Notion status 그룹상 `컨테이너`는 `To-do` 그룹에 포함되었다.
3. 다음 Project를 `진행 상황 = 컨테이너`로 변경했다.
   - `WEB`
   - `그로쓰/연구`
   - `온디`
   - `웹에이전시`
...
1. 검증 중 `관계`라는 Inbox Project가 새로 보였다.
2. 이 항목은 이번 요청 대상이 아니므로 수정하지 않았다.
3. 컨테이너성 Project는 목표형 Project처럼 완료 처리하지 않는다.
4. 컨테이너성 Project의 노출 여부는 `Hide`로 별도 조절한다.
